...
export class PaymentCompareComponent implements OnInit {
  ...
  ngOnInit() {
    this.loanservice.cast.subscribe((data) => {
      ...
      this.chartLabels = this.paymentservice.buildPaymentDates(
      ...
      this.ttlloanAmnt1 = this.paymentservice.buildTotalLoanAmntwInt(
        this.payments1,
        this.pricing.loanAmnt
      );
      this.ttlloanAmnt2 = this.paymentservice.buildTotalLoanAmntwInt(
        this.payments2,
        this.pricing.loanAmnt
      );
      this.ttlloanAmnt3 = this.paymentservice.buildTotalLoanAmntwInt(
        this.payments3,
        this.pricing.loanAmnt
      );
      this.ttlInterest1 = this.paymentservice.formatCurrency(
        Math.max(...this.payments1.map((d) => d.TotalInt))
      );
      this.ttlInterest2 = this.paymentservice.formatCurrency(
        Math.max(...this.payments2.map((d) => d.TotalInt))
      );
      this.ttlInterest3 = this.paymentservice.formatCurrency(
        Math.max(...this.payments3.map((d) => d.TotalInt))
      );
      // console.log('chartLabels ',this.chartLabels);
      if (this.lineChart) {
        this.lineChart.data.labels = this.chartLabels;
        this.lineChart.data.datasets[0].data = this.payments1.map(
          (d) => d.cumpayment
        );
        this.lineChart.data.datasets[1].data = this.payments2.map(
          (d) => d.cumpayment
        );
        this.lineChart.data.datasets[2].data = this.payments3.map(
          (d) => d.cumpayment
        );
